...
float exp2cursor(float cursor, int octaves)
{
    int useOctaves = octaves?octaves:kRandFloatExp2Limit;
    if(cursor<2.0/((float)useOctaves))
        return cursor*useOctaves/2.0 * powf(2.0,(2.0/((float)useOctaves) - 1.0)* useOctaves);
    else
        return powf(2.0,(cursor - 1.0)* useOctaves);
}
