...
public:
    ...
    void PlaySeperator(int depth);
    void PlayIntArray(int * array, int size, float duration);
    float PlayDoubleArray(double * array, int size);
	...
	struct ssortdata* GetSortData(){return &mzsortdata;}
    //to be used whenever the sortdata which is sent to the audio thread is changed.
    void LockSortData();
    void unLockSortData();
    
    void ResetEvalCount(){mzsortdata.approxEvals =0;}
    int  GetEvalCount(){return mzsortdata.approxEvals;}
